/**
 * ElectionGuard 2.1 standard baseline cryptographic parameters.
 *
 * These are the 4096-bit standard parameters from the ElectionGuard Design
 * Specification v2.1.0. In v2.1 the large prime `p` and cofactor `r` were
 * regenerated using ln(2) (replacing the Euler-Mascheroni constant used in
 * v2.0) and `p` was moved further from 2^4096.
 *
 * Source: microsoft/electionguard-rust `standard_parameters.rs`
 * (https://github.com/microsoft/electionguard-rust) — the reference
 * implementation tracking Design Spec v2.1.0.
 *
 * These constants are NOT secret. Their correctness is checked at load time
 * by `validateGroup()` in `group.ts` and by the conformance harness — the
 * algebraic relation p = q·r + 1 and the subgroup check g^q mod p = 1 catch
 * any transcription error. Do not edit by hand without re-running the tests.
 *
 * Whitespace below is purely for auditability against the spec's 8-column
 * hex grouping; it is stripped when parsed.
 */

/** Small prime `q` — the order of the ElGamal subgroup (256-bit, = 2^256 − 189). */
export const Q_HEX = `
FFFFFFFF FFFFFFFF FFFFFFFF FFFFFFFF FFFFFFFF FFFFFFFF FFFFFFFF FFFFFF43
`;

/** Large prime `p` — the modulus (4096-bit). */
export const P_HEX = `
...
/** Generator `g` — generates the order-`q` subgroup of Z*_p. */
export const G_HEX = `
...
/** Identifier for this parameter set, used in election records. */
export const EG_2_1_PARAMETER_SET = 'ElectionGuard-2.1.0-standard-4096';
